      ...
      this.$confirm('确定要提交评论吗？', '提示', {
        ...
        publishComment(that.reply, this.$store.state.token).then(data => {
          if (data.success) {
            ...
            this.reply.content = ''; // 清空输入框
            this.commentShow = false; // 隐藏评论输入框
            window.location.reload(); // 刷新页面
          } else {
            this.$message({ type: 'error', message: '评论失败: ' + data.msg, showClose: true });
          }
        }).catch(error => {
          this.$message({ type: 'error', message: '评论失败: ' + error, showClose: true });
        });
      }).catch(() => {
        // 用户点击了取消
        this.$message({
